var amoAd = (function(){
  var ctaAlignment = function(position){
    ...
    layers.cta.arrow.x = (adWidth - ctaTextWidth)/2 + ctaTextWidth + 2;
    layers.txtCta.children[0].y += 1;
    layers.cta.arrow.y += 0.75;
  }
  ...
  function init() {
    ...
    bannerData.headline3 = eval(bannerData.textField3.split('^')[0]);
    bannerData.headline4 = eval(bannerData.textField4.split('^')[0]);
    ...
    bannerData.ctaText = bannerData.ctaText.replace('<br>', '\n');
  	...
  	CTAFont = (Boolean(parseFloat(CTAFont)) ? parseFloat(CTAFont) : 16) + 'px';
  	bannerData.ctaText = bannerData.ctaText.split('|')[0];
  	bannerData.CTA = bannerData.ctaText ? [bannerData.ctaText,CTAFont,0,0,"21","300", "left", "Segoe Pro", "#FFFFFF"] : '';
    window.smartNames = [];
    ...
    (bannerData.textField3.split('^')[1]) ? window.smartNames.push(bannerData.textField3.split('^')[1]) : [];
    (bannerData.textField4.split('^')[1]) ? window.smartNames.push(bannerData.textField4.split('^')[1]) : [];
    (bannerData.textField5.split('^')[1]) ? window.smartNames.push(bannerData.textField5.split('^')[1]) : [];
    resizeCTA();
    fireImpression();
  ...
  function resizeCTA() {
    // var layers = exportRoot.mainMC;
  	...
  	// var ctaHeight = Math.ceil(Math.abs(yTemp[0]) + Math.abs(yTemp[1]));
  	// ctaHeight *= ctaScaleY * layers.cta.children[0].scaleY;
    var ctaMaxWidth = 180, ctaMaxHeight = 44;
