var amoAd = (function(){
  ...
  function init() {
    ...
    bannerData.ctaText = bannerData.ctaText.replace('<br>', '\n');
  	...
  	CTAFont = (Boolean(parseFloat(CTAFont)) ? parseFloat(CTAFont) : 20) + 'px';
  	bannerData.ctaText = bannerData.ctaText.split('|')[0];
  	bannerData.CTA = bannerData.ctaText ? [bannerData.ctaText,CTAFont,0,0,"27","300", "left", "Segoe Pro", "#FFFFFF"] : '';
    window.smartNames = [];
  ...
  function resizeCTA() {
    // var layers = exportRoot.mainMC;
  	...
  	// var ctaHeight = Math.ceil(Math.abs(yTemp[0]) + Math.abs(yTemp[1]));
  	// ctaHeight *= ctaScaleY * layers.cta.children[0].scaleY;
    var ctaMaxWidth = 180, ctaMaxHeight = 56;
