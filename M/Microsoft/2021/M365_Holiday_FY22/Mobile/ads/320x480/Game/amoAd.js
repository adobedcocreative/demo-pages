var amoAd = (function(){
  ...
  var ctaAlignment = function(position){
    ...
    var txtLearnCTAWidth = Math.round(layers.txtLearn.children[0].children[0].getBounds().width);
    ...
    layers.cta1.arrow.x = (adWidth - txtLearnCTAWidth)/2 + txtLearnCTAWidth + 5;
    layers.txtLearn.x = (adWidth - txtLearnCTAWidth)/2 - 5;
    layers.txtLearn.regY = 0;
    layers.cta1.arrow.y = -4;
    layers.cta.arrow.y = 2;
    layers.txtLearn.children[0].x = 0;
		//layers.cta1.arrow.regX = 0;
    // exportRoot.mainMC.cta1.arrow.regY += 1;
		// layers.cta1.arrow.x = (adWidth - layers.txtLearn.children[0].children[0].getBounds().width)/2 + layers.txtLearn.children[0].children[0].getBounds().width + 2;
    //layers.txtLearn.regX = 0;
  //   var layers = exportRoot.mainMC;
  //   var adWidth = document.querySelector('#animation_container').offsetWidth;
  ...
  //   position = position ? position : (layers.cta.x > adWidth/ - 5 ? 'right' : 'left');
  ...
  function init() {
    ...
    if(typeof bannerData.ctaText !== "object") {
      bannerData.ctaText = bannerData.ctaText.split(',');
    }
    bannerData.ctaText.forEach(function(elem,i) {
      bannerData.ctaText[i] = elem.trim().replace('<br>','\n');
      var CTAFont = bannerData.ctaText[i].split('|').length > 1 ? bannerData.ctaText[i].split('|')[1] : '0';
  	  CTAFont = (Boolean(parseFloat(CTAFont)) ? parseFloat(CTAFont) : 14) + 'px';
    });
    bannerData.CTA = bannerData.ctaText[1] ? addDynamicText(bannerData.ctaText[1], ["Shop now",'15px',0,0,"19","300", "right", "Segoe Pro", "#FFFFFF"]) : '';
    bannerData.CTA1 = bannerData.ctaText[1] ? addDynamicText(bannerData.ctaText[1], ["Shop now",'15px',0,0,"8","300", "right", "Segoe Pro", "#058475"]) : '';
    bannerData.replaybtn = bannerData.ctaText[2] ? addDynamicText(bannerData.ctaText[2], ["Play again",'15px',0,0,'19','300', 'left','Segoe Pro', '#FFFFFF']) : '';
    bannerData.playTxt = bannerData.ctaText[0] ? addDynamicText(bannerData.ctaText[0], ["PLAY NOW",'14px',0,0,'16','300', 'center','Segoe Pro', '#FFFFFF']) : '';
    resizeCTA();
    fireImpression();
  ...
  function resizeCTA() {
    var layers = exportRoot.mainMC;
  	...
  	var ctaHeight = Math.ceil(Math.abs(yTemp[0]) + Math.abs(yTemp[1]));
  	ctaHeight *= ctaScaleY * layers.cta.children[0].scaleY;
    var ctaMaxWidth = 160, ctaMaxHeight = ctaHeight;
