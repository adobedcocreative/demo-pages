// JavaScript Document
	//  TEXT, TEXTSIZE, XPOS, YPOS, LINE SPACING, LINE WIDTH, JUSTIFYTEXT
	// use | for new line.  Use <#FFFFFF> for color change.
	// when this occurs you can use line height to adjust line spacing.
	// lineWidth is no longer needed.
	var bannerData = new Object()
	
	bannerData.headline1 = ["<#FFFFFF>Start fresh with |Microsoft 365","22px",150, 76,"25","3 50", "center"];
	bannerData.headline2 = ["<#FFFFFF>Start reaching your money |goals with smart features","12px",150, 130,"14","3 50", "center"];
	bannerData.CTA = ["<#107C41>LEARN MORE","12px",-1,0,"50","300", "center"];
	
	bannerData.CTAarrowVisible = [true, 0,0]
