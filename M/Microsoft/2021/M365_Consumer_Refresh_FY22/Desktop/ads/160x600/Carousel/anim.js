function frame0(){
    ...
    exportRoot.runBanner = function() {
        ...
        exportRoot.gotoNextNav = function(type) {
            if(type===undefined) {
                ...
                //for moving the nav
                navHeight = nav.icon1.y+nav.icon5.y+64;
                centerNavPosition = 463;
                if (exportRoot.currentSelection==1) centerNavPosition = 418;
                if (exportRoot.currentSelection==5) centerNavPosition = 510;
                movingGap = centerNavPosition - nav["icon" + exportRoot.currentSelection].y;
            ...
            if(type==1) {
                for (var i = 2; i <= maxNav; i++) {
                    gsap.to(nav["icon" + i], 1, {scale: .772, ease: Power4.easeInOut});
                }
            }
        }
	...
	//logo animation
		...
		exportRoot.nextScene = function() {
			...
			for (i=0; i<exportRoot["headline"+exportRoot.currentSelection].length; i++) {
                exportRoot.tlNext.to(exportRoot["headline" + exportRoot.currentSelection][i], 0, {
                    y: init_headline_y + initOffset / 5 + 20*i,
                    alpha: 0
                });
            }
            ...
            for (i=0; i<exportRoot["headline"+prevSelection].length; i++) {
                exportRoot.tlNext.to(exportRoot["headline" + prevSelection][i], 0.4, {
                    y: init_headline_y - initOffset / 3 + 20*i,
                    alpha: 0,
                    ease: Power2.easeIn,
                    //stagger: 0.03
                }, "<");
        ...
        //1st icons animation
        exportRoot.tlText.to(exportRoot.mainMC.nav.icon5, .6, { x:"+=0", scaleX:.53, scaleY:.53, ease:Power1.easeInOut},"<-1")
            .to(exportRoot.mainMC.nav.icon5, .3, {y:606, ease:Power1.easeInOut},"<+.3")
            .to(exportRoot.mainMC.nav.icon4, .7, { x:"+=0", scaleX:.53, scaleY:.53, ease:Power1.easeInOut},"<-.2")
            .to(exportRoot.mainMC.nav.icon4, .3, {y:559, ease:Power1.easeInOut},"<+.4")
            .to(exportRoot.mainMC.nav.icon3, .8, { x:"+=0", scaleX:.53, scaleY:.53, ease:Power1.easeInOut},"<-.3")
            .to(exportRoot.mainMC.nav.icon3, .4, {y:513, ease:Power1.easeInOut},"<+.5")
            .to(exportRoot.mainMC.nav.icon2, .9, { x:"+=0", scaleX:.53, scaleY:.53, ease:Power1.easeInOut},"<-.4")
            .to(exportRoot.mainMC.nav.icon2, .5, {y:467, ease:Power1.easeInOut},"<+.4")
            .to(exportRoot.mainMC.nav.icon1, 1, { x:"+=0", scale:.76, ease:Power1.easeInOut},"<-.3")
            .to(exportRoot.mainMC.nav.icon1, .6, {y:419, ease:Power1.easeInOut, onComplete:function(){
                timer1.play();
