...
function frame0() {
    ...
    exportRoot.fillAllText = function (typeTmp, txtDetails, objTmp, aVar) {
		...
		// if (typeObj==0 || typeObj==1 ) {
			...
			for (var i = 0; i < aSentenceLine.length; i++) {
				...
				if (typeObj==2) {objTmp.addChild(mc2)} 
				objTmp.addChild(mc);
		...
		//if (typeObj==2) console.log("looping")
			if (typeObj==2) distanceArray4Cursor.push(textArray)
		// } 
	}
    ...
    exportRoot.runBanner = function() {
		...
		exportRoot.typewrite1[0].alpha 


		var init=0
		for (var i=0; i < distanceArray4Cursor.length; i++) {
			timeDelay = (i==0) ? timeDelay=2 : timeDelay=0.1
			init=0
			var cycle=i
			this.tlAnim.from(exportRoot["typewrite"+(i+1)],{
				data: new Array(i, exportRoot["typewrite"+(i+1)][1].xOffset),
				duration:0.01,
				alpha: 0,
				stagger: {
					each:0.05,
					onComplete() {
						var pos = distanceArray4Cursor[this.data[0]][init]+(this.data[1]+8)
						var finalpos = (isNaN(pos)) ? distanceArray4Cursor[this.data[0]][init-1]+(this.data[1]+8) : pos
						exportRoot["typewrite"+(this.data[0]+1)][0].x = finalpos
						init++
					}},
				ease:Power4.easeOut,
				onStart:function(){
					exportRoot["typewrite"+(this.data[0]+1)][0].alpha = 1,
					init=0,
					exportRoot["typewrite"+(this.data[0]+1)][0].y = exportRoot["typewrite"+(this.data[0]+1)][1].y },
				onComplete:function(){
					exportRoot["typewrite"+(this.data[0]+1)][0].alpha = 0
			}}, ("+="+timeDelay));
		}
		
		exportRoot.cursormc = exportRoot["typewrite"+(distanceArray4Cursor.length)][0]
		//Flash begin

		this.tlAnim.to(exportRoot.cursormc,{ duration:0.01, alpha: 1, ease:Power3.easeIn}, "+=0.3");
		this.tlAnim.to(exportRoot.cursormc,{ duration:0.01, alpha: 0, ease:Power3.easeIn}, "+=0.3");
		this.tlAnim.to(exportRoot.cursormc,{ duration:0.01, alpha: 1, ease:Power3.easeIn}, "+=0.3");
		this.tlAnim.to(exportRoot.cursormc,{ duration:0.01, alpha: 0, ease:Power3.easeIn}, "+=0.3");
		this.tlAnim.to(exportRoot.cursormc,{ duration:0.01, alpha: 1, ease:Power3.easeIn}, "+=0.3");
		this.tlAnim.to(exportRoot.cursormc,{ duration:0.01, alpha: 0, ease:Power3.easeIn}, "+=0.3");
		...
		for (var i=0; i < distanceArray4Cursor.length; i++) {
			timeDelay = (i==0) ? timeDelay="+=0.8" : timeDelay="-=0.6" 
			this.tlAnim.to(exportRoot["typewrite"+(i+1)],{
				duration:0.7,
				alpha: 0,
				ease:Power3.easeIn,
        ...
        this.tlAnim.from(mc.replay_btn, 1, { alpha: 0, onStart:function(){exportRoot.isReplay = true;}}, "-=0.5");
