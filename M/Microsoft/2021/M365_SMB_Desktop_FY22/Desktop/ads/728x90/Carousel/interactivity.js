...
function changeSlide() {
    ...
    cst.to(exportRoot.mainMC,{duration:0.5, onStart: function() {
        ...
        .to(exportRoot.mainMC.slides,{duration:0.5, overwrite:true, scale:zoomFactor,
            ...
            onStart: function () {
                navColor(0);
                for(s=1; s<=exportRoot.nSlide; s++) {
                    gsap.to(exportRoot.mainMC.slides.sub_slides["slide"+s], {duration:0.5, alpha:1, overwrite:"auto"})
                }
            },
        ...
        .to(exportRoot.mainMC.slides,{duration:0.5, scale:1, x:0, y:0, ease:"power2.inOut",
            onStart: function () {
                navColor(1);
                for(s=1; s<=exportRoot.nSlide; s++) {
                    if(s!=exportRoot.selectedSlide) gsap.to(exportRoot.mainMC.slides.sub_slides["slide"+s], {duration:0.5, alpha:0})
                }
            }
        },"<.8")
...
function slideMaskAnimation(oC,slideN) {
    ...
    if(oC==1) {
        ...
        gsap.to(exportRoot.mainMC.bg, {duration:1, alpha:0, delay:0.5});
    }
    if(oC==0) {
        for(i=1; i<exportRoot.nSlide+1; i++) {
            ...
            gsap.to(exportRoot.mainMC.bg, {duration:0.3, alpha:1, overwrite:true});
