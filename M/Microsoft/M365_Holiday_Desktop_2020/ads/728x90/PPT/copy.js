// JavaScript Document
	//  TEXT, TEXTSIZE, XPOS, YPOS, LINE SPACING, LINE WIDTH, JUSTIFYTEXT
	// use | for new line.  Use <#FFFFFF> for color change.
	// when this occurs you can use line height to adjust line spacing.
	// lineWidth is no longer needed.
	var bannerData = new Object()
	
	bannerData.headline1 = ["<#a80000>Personalize your season's greetings","18.4px",311 , 19,"20","3 50", "left"];
	bannerData.headline2 = ["<#505050>Customize and create a warm greeting with|smart templates","12.2px",311, 44,"14","3 50", "left"];

	bannerData.CTA = ["<#FFFFFF>BUY NOW","12px",0,0,"50","728", "center"];
	
	bannerData.CTAarrowVisible = [true, 0,0]
