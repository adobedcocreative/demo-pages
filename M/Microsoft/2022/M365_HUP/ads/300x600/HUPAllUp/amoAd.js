var amoAd = (function(){
  var ctaAlignment = function(position){
    ...
    var ctaHeight = Math.ceil(Math.abs(yTemp[0]) + Math.abs(yTemp[1])) + 5;
  ...
  //   position = position ? position : (layers.cta.x > adWidth/2 ? 'right' : 'left');
    ...
    layers.cta.arrow.arrow.children[0].x = 0;
    ...
    layers.cta.children[0].graphics.beginFill(ctaBackgroundColor).drawRoundRectComplex(-ctaWidth/2, -ctaHeight/2, ctaWidth, ctaHeight, 7, 7, 7, 7);
    //var ctaTextWidth = layers.txtCta.children[0].children[0].getBounds().width + 15;
    layers.txtCta.x = (adWidth)/2 - ctaTextWidth/2;
    layers.cta.arrow.x = ctaTextWidth/2 - 8;
  ...
  function init() {
    ...
    bannerData.CTA = bannerData.ctaText ? [bannerData.ctaText,CTAFont,32,540,'17','300', 'left', 'SegoeSemi', '#0078d3'] : '';
    // resizeCTA();
    fireImpression();
